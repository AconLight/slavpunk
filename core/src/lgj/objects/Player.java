...
public class Player extends GameObject {
    ...
    public void act(float delta) {
        ...
        if (!isAddedToC1) {
            isAddedToC1 = true;
            ((CannonBase) GameObjectManager.gameObjects.get("cannonBase1")).parts.add(30000);
            ((CannonBase) GameObjectManager.gameObjects.get("cannonBase1")).playerIds.add(id);
        }

        if (!isAddedToC2) {
            isAddedToC2 = true;
            ((CannonBase) GameObjectManager.gameObjects.get("cannonBase2")).parts.add(30000);
            ((CannonBase) GameObjectManager.gameObjects.get("cannonBase2")).playerIds.add(id);
        }

        if (!isAddedToC3) {
            isAddedToC3 = true;
            ((CannonBase) GameObjectManager.gameObjects.get("cannonBase3")).parts.add(30000);
